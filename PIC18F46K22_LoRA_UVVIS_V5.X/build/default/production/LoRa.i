...
uint8_t LoRaGetVersion(){
uint8_t temp = SPI2ReadByte(0x42);
return temp;
}
...
writeOpModeRegister(regValue);
}

uint8_t readOpModeRegister(){
return SPI2ReadByte(0x01);
}

void writeOpModeRegister(uint8_t regValue){
SPI2WriteByte(0x01, regValue);
}



void LoRaMode_RXActive(){
SPI2WriteByte(0x01, 0b1000101);
}
...
SPI2WriteByte(0x0D, 0);
SPI2WriteByte(0x22, 0);


for(uint8_t i=0;i<dataLength;i++){
SPI2WriteByte(0x00, data[i]);
}
SPI2WriteByte(0x22, dataLength);
LoRaTXMode();
if(0){
printf("Done.\r\n");
}
...
uint8_t msb = (intermediate>>16) & 0xFF;
uint8_t mid = (intermediate>>8)& 0xFF;
uint8_t lsb = intermediate & 0xFF;

SPI2WriteByte(0x06,msb);
SPI2WriteByte(0x07,mid);
SPI2WriteByte(0x08,lsb);
}

# 234
float LoRaGetFrequency(){
uint8_t msb = SPI2ReadByte(0x06);
uint8_t mid = SPI2ReadByte(0x07);
uint8_t lsb = SPI2ReadByte(0x08);
...
uint8_t LoRaGetIRQFlags(){
uint8_t regValue = SPI2ReadByte(0x12);
return regValue;
}

void LoRaClearIRQFlags(){
SPI2WriteByte(0x12,0xFF);
}
